...
class _MatrixGestureDetectorState extends State<MatrixGestureDetector> {
  ...
  @override
  void initState() {
    translationUpdater.value = Offset.zero;
    rotationUpdater.value = double.nan;
    scaleUpdater.value = 1.0;

    if (widget.initialTranslate != null) {
      print("t");
      Offset t = Offset(widget.initialTranslate.dx,widget.initialTranslate.dy);
      //if(widget.initialScale != null) t = t.scale(9,9);
      matrix = _translate(t) * matrix;
      print(matrix);
    }
    if (widget.initialScale != null) {
      matrix = _scale(widget.initialScale, widget.initialTranslate ?? Offset.zero) * matrix;
    }
    Timer(Duration(microseconds: 1), () => widget.onMatrixUpdate(
  ...
  void onScaleStart(ScaleStartDetails details) {
    ...
    if (widget.onPanStart != null) {
      widget.onPanStart(
          MatrixGestureDetector.fromViewport(details.localFocalPoint, matrix).translate(0, 0));
    }
  ...
  void onScaleUpdate(ScaleUpdateDetails details) {
    ...
    if (details.scale == 1 && widget.onPanUpdate != null) {
      widget.onPanUpdate(
          MatrixGestureDetector.fromViewport(details.focalPoint, matrix).translate(0, 0));
      return;
    }
